...
///*
// Bellow is my spaghetti that I've been so lovingly cooking for the last 4 day
// Enjoy!
// Vadim K
// 13/09/2022
//*/
...
object Calculator {
    ...
    private val assignMap = mutableMapOf<String, BigInteger>()
    ...
    private fun inputToList(input: String) : MutableList<String> {
        ...
        try {
            if (list[0] == "-" && list[1].matches("\\w+".toRegex())) {
                list.removeAt(0)
                list[0] = (list[0].toInt() * -1).toBigInteger().toString()
        ...
        try {
            ...
            for (i in list.indices) {
                ...
                if (list[i] == "-" && list[i - 1].matches("[+*/-]".toRegex())) {
                    list[i + 1] = (list[i + 1].toInt() * -1).toBigInteger().toString()
                    list.removeAt(i)
                }
            }
    ...
    private fun assignValue(input: String): MutableMap<String, BigInteger> {
        ...
        val value: BigInteger =
            if (assignMap.containsKey(inputRightOfEquals)) { // if trying to assign a letter that is already in map, then its value is assigned
                ...
                inputRightOfEquals.split("\\s+".toRegex()).toMutableList().joinToString("").toBigInteger() // whitespace removed and number is assigned to value (or exception is thrown and caught)
            }
    ...
    private fun postfixCalculation(inputToList: MutableList<String>) {
        ...
        for (i in postfixExpression.indices) {
            if (postfixExpression[i].matches(numberRegex)) {
                ...
                val resultOfLastTwoNumbersPopped = when (postfixExpression[i]) {
                    "+" -> stack[stack.lastIndex - 1].toBigInteger() + stack.last().toBigInteger()
                    "-" -> stack[stack.lastIndex - 1].toBigInteger() - stack.last().toBigInteger()
                    "*" -> stack[stack.lastIndex - 1].toBigInteger() * stack.last().toBigInteger()
                    "/" -> stack[stack.lastIndex - 1].toBigInteger() / stack.last().toBigInteger()
        ...
        println(stack.joinToString("").toBigInteger()) //the answer is the only remaining number in stack
    }
    ...
    private fun operation(input: String) {
        ...
        if (inputToList.joinToString("").matches(regex) && bracketsAreClosed) {
            ...
            println(input.trim().toBigInteger())
